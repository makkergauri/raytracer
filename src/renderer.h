...
struct RenderSettings {
    ...
    int   max_depth         = 12;
    Color sky_top           = Color(0.50f, 0.70f, 1.00f);
...
inline Color ray_color(const Ray& r, const Hittable& world,
    ...
    if (!world.hit(r, 0.001f, INF, rec)) {
        if (cfg.env && cfg.env->valid()) return cfg.env->sample(r.dir);
    ...
    if (bounce > 4) {
        float p = clampf(std::fmax(attenuation.x, std::fmax(attenuation.y, attenuation.z)), 0.05f, 1.0f);
...
inline void render(const Hittable& world, const Camera& cam,
    ...
    for (int j = 0; j < cfg.height; ++j) {
        for (int i = 0; i < cfg.width; ++i) {
            ...
            col = gamma_correct(aces_tonemap(col));
